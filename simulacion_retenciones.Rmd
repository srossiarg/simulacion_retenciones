---
title: "Retenciones vs Imp. a las Ganancias: impacto sobre el patrimonio de los productores"
...
* Escenario 3 **'Aumento de Ganancias'**: 25% de alícuota de retención y 35% de alícuota de impuesto a las ganancias.
...
Como el clima afecta directamente los rendimientos, en nuestras simulaciones asumimos que los rendimientos presentan una distribución normal con media 30 quintales y desvío de 9 quintales. 
...
sim_soja <- function(rinde_soja=3,desvio_soja=0.9, costos_usd_ha=500,
                     n_sims = 10000,precio_soja = 330,desvio_precio = 40,
                     alicuota_retencion=0.3, tasa_ganancias=0.35,n_sim=100) {
  
  #Seteo condiciones iniciales
  patrimonio = 3 * costos_usd_ha #Patrimonio en to permite costear tres campañas
  precio_fas = precio_soja*(1-alicuota_retencion)
  pneto =c()
  reca =c()
  resultado =c()
  prod=c()
  ...
  while(patrimonio>0 & j<n_sim) {
      ...
      arrendamiento = (rinde_soja/4) * precio_fas
...
*  25% de Alícuota de retención y 25% de impuesto a las ganancias.
```{r, echo=FALSE}
#Caso 1: situacion actual
set.seed(2019)
res1 <- sim_soja(n_sim = 1000,alicuota_retencion = 0.25, tasa_ganancias = 0.25)
```
...
escenario_1<-data.frame("escenario" =1,
                        "alicuota_retencion (%)" =0.27,
                        "tasa_ganancias_(%)" = 0.25,
...
* el productor pierde todo su patrimonio al cabo de **62 campañas**
* el gobierno recauda, a lo largo de las 62 campañas, **USD 11.985** por hectárea
* El productor **tiene un resultado positivo el 43% de las campañas**
* En promedio el productor **pierde 25 dólares por hectárea cada campaña**
...
* 35% de Alícuota de retención y 25% de impuesto a las ganancias.

```{r,echo=FALSE}
#Caso 2: suben soja, mantienen ganancias
set.seed(2019)
res2 <- sim_soja(n_sim = 1000,alicuota_retencion = 0.35, tasa_ganancias = 0.25)
```
...
* el productor pierde todo su patrimonio al cabo de **14 campañas**
* el gobierno recauda, a lo largo de las 14 campañas, USD 2946 por hectárea
* El productor tiene un **resultado positivo solo el 35%** de las campañas
* En promedio el productor **pierde 113 dólares por hectarea cada campaña**

#### Simulación Escenario 3
```{r,echo=FALSE, warning=FALSE}
#Caso 3: = retenciones soja, aumentas ganancias
set.seed(2019)
res3 <- sim_soja(n_sim = 1000,alicuota_retencion = 0.25, tasa_ganancias = 0.35)
...
* el productor pierde todo su patrimonio al cabo de **60 campañas**
* el gobierno recauda, a lo largo de las 60 campañas, USD 12164 por hectárea
* El productor tiene un **resultado positivo el 43.3%** de las campañas
* En promedio el productor **pierde 29 dólares por hectarea cada campaña**
...
ggplot(res, aes(y = pneto, x = periodo , colour = as.factor(escenario) )) +
  ...
  labs(x = "Campañas", y = "Patrimonio Neto del productor (en dólares)", colour= "") +
...
ggplot(res, aes(y = reca, x = periodo , colour = as.factor(escenario) )) +
  ...
  labs(x = "Nº de Campañas hasta quebranto", y = "Recaudacion acumulada (en USD)",subtitle = "en dólares", colour= "") +
  theme(axis.line.x = element_line(size = .5, colour = "black"),
        axis.line.y = element_line(size = .5, colour = "black"),
        axis.text.x = element_text(colour = "black", size = 10, angle = 90),
        axis.text.y = element_text(colour = "black", size = 10),
...
Sobre este punto se destaca que el escenario 2 (mayores retenciones) presenta una distribución bimodal muy acentuada. Esto significa que los resutlados económicos en ese escenario son estan mas polarizados. Esto se debe al caracter prociclico de las retenciones.
...
ggplot(res, aes(  y = escenario , x = resultado_economico, fill = as.factor(escenario) )) +
  ...
  labs(x = "Resultado Económicos (en USD)", y = "Frecuencia", fill= "") +
  theme(axis.line.x = element_line(size = .5, colour = "black"),
        axis.line.y = element_line(size = .5, colour = "black"),
...
En el gráfico siguiente se presenta la proporción de campañas con resultado económico positivo como proporción del total de campañas. El gráfico muestra claramente como el escenario de mayores retenciones hace que cerca del 60% de las campañas arrojen pérdidas, mientras que en el caso de mayores impuestos a las ganancias la proporción de campañas con pérdidas es idéntica a la situación actual (benchmark).

En este sentido, se puede destacar que incrementar el impuesto a las ganancias permite incrementar la recaudación sin poner en riesgo la sustentabilidad económica del sector agrícola. 
...
ggplot(tab_contingencia, aes(y = valor*100, x = escenario , fill = as.factor(variable) )) +
  geom_col() + 
  labs(title = "Ratio campañas con ganancia/ pérdidas",
       subtitle = " ") +
  theme_bw() + scale_fill_fivethirtyeight() +
  coord_flip()+
...
* Un incremento de las alícuotas de retención a 35% puede llevar a un brusco deterioro de la situación patrimonial de los productores. 
